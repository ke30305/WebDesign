def notifySlack(String status) {
    ...
    def commitMsg  = sh(script: 'git log -1 --pretty=format:"%s - %an"', returnStdout: true).trim()
    def commitLog  = "`${commitHash}` - ${commitMsg}"
    ...
    sh 'curl -s -X POST -H "Content-type: application/json" -d @/tmp/slack_payload.json "${SLACK_WEBHOOK}"'
}
